...
export function EmployeeLeave() {
        ...
        const [leaveType, setLeaveType] = useState<NonNullable<LeaveRequest['leaveType']>>('CASUAL')
        ...
        useEffect(() => {
            ...
            async function load() {
                ...
                try {
                    ...
                    const data = await leaveService.getLeavesByEmployeeId(id)
                    if (!mounted) return
                    setLeaves(data)
                } catch (e) {
                    console.error('Leave load error', e)
                    if (!mounted) return
                    setError('Unable to load leave applications.')
                } finally {
                    if (!mounted) return
                    setLoading(false)
                }
        ...
        const handleSubmit = async (event: FormEvent) => {
            ...
            setError('')
            try {
                const created = await leaveService.applyLeave(employeeId, { leaveType, startDate, endDate, reason })
